...
# SPECIFICATION: This program will read the optdigits.tra file in order to
# build a Single Layer Perception and Multi-Layer Perception classifiers. Simulate a
# grid search and test accuracy of the classifier using the optdigits.tes file.
...
for w in n: #iterates over n
    ...
    for b in r: #iterates over r
        ...
        for a in range(2): #iterates over the algorithms
            ...
            #--> add your Python code here
            if a == 0:
                if acc > highPerceptionAccuracy:
                    highPerceptionAccuracy = acc
                    print("Highest Perception Accuracy so far: " + str(highPerceptionAccuracy) + ", Parameters: learning rate=" + str(w) + ", shuffle=" + str(b))
            else:
                if acc > highMultiLayerPerceptionAccuracy:
                    highMultiLayerPerceptionAccuracy = acc
